<img width="2525" height="1355" alt="image" src="https://github.com/user-attachments/assets/1f29e00a-1592-412f-b190-51671a21dac6" />
<img width="2499" height="1358" alt="image" src="https://github.com/user-attachments/assets/4b75f60b-3989-4b65-af8d-a7df55ab9482" />
...
```
## 可移動序列版本01
<img width="1556" height="585" alt="image" src="https://github.com/user-attachments/assets/3c0aea11-d68b-47c7-a826-05b81a0ae377" />
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Firebase 圖文網格編輯範例 (可左右移動順序)</title>
    ...
    <style>
        ...
        #public-display ul li {
            ...
            flex-direction: column;
            justify-content: space-between;
            position: relative; /* 🚨 讓 li 成為按鈕定位的基準 🚨 */
        }
        ...
        .action-btn {
            ...
            transition: background-color 0.2s;
            flex-shrink: 0;
            margin: 5px auto 0 auto !important; /* 確保刪除按鈕依然置中 */
        }
...
<div class="container">
    <h1><i class="fas fa-globe-asia"></i> 公開內容展示</h1>
    <p>以下為成員圖文列表，管理員可使用「左右箭頭」調整順序。</p>
...
<script>
    ...
    // ** 4. 讀取公開內容 (即時監聽：網格渲染) **
    PUBLIC_CONTENT_COLLECTION_REF
        .orderBy("timestamp", "asc") // 排序依據 timestamp 升序
        .limit(30) // 增加限制，確保看到更多成員
        .onSnapshot((snapshot) => {
            const isLoggedIn = auth.currentUser !== null; // 檢查登入狀態
            let html = '<ul style="list-style: none; padding: 0;">';
            ...
            snapshot.docs.forEach((doc, index) => { 
                ...
                const docId = doc.id; 
                const nickname = data.text.trim() || '未命名'; 
                const currentOrder = index + 1; // 列表中的實際位置
                const totalItems = snapshot.docs.length; // 總成員數
                ...
                html += `
                    <li>
                        ${isLoggedIn && currentOrder > 1 ? 
                            `<button class="action-btn move-btn left" onclick="moveContent('${docId}', 'up', ${index})">
                                <i class="fas fa-angle-left"></i> 
                            </button>` : ''}
                            
                        ${isLoggedIn && currentOrder < totalItems ? 
                            `<button class="action-btn move-btn right" onclick="moveContent('${docId}', 'down', ${index})">
                                <i class="fas fa-angle-right"></i>
                            </button>` : ''}

                        <p class="member-id">成員 ${currentOrder}</p> ${imageHtml}
                        <p class="member-name">${nickname}</p>
                        
                        ${isLoggedIn ? 
                            `<button class="action-btn delete-btn" onclick="deleteContent('${docId}', '${data.imageUrl || ''}')">
                                <i class="fas fa-trash-alt"></i> 刪除
                            </button>` : ''}
                    </li>
                `;
    ...
    // ** 7. 移動內容函數 (Move Left/Right 實際上是 Up/Down) **
    window.moveContent = async (docId, direction, currentIndex) => {
        if (!auth.currentUser) {
            alert('您沒有權限執行此操作。');
            return;
        }
        
        // 🚨 修正方向：'up' -> 'left' (向小 timestamp 移動) / 'down' -> 'right' (向大 timestamp 移動) 🚨
        const moveDirection = direction === 'up' ? -1 : 1; 

        // 重新讀取當前列表的最新狀態（確保順序正確）
        const snapshot = await PUBLIC_CONTENT_COLLECTION_REF
        ...
        if (targetIndex < 0 || targetIndex >= docs.length) {
            return; // 已經在頂部或底部，不能移動
        }

        // 取得目標項目的 document ID 和 timestamp
        const targetDoc = docs[targetIndex];
        const targetTimestamp = targetDoc.data().timestamp;
        
        // 取得當前項目的 document ID 和 timestamp
        const currentDoc = docs[currentIndex];
        const currentTimestamp = currentDoc.data().timestamp;

        saveMessage.textContent = `正在移動成員...`;

        try {
            // 核心邏輯：交換兩個 document 的 timestamp 欄位
            
            // 1. 更新目標項目的 timestamp 為當前 timestamp
            await PUBLIC_CONTENT_COLLECTION_REF.doc(targetDoc.id).update({
                timestamp: currentTimestamp
            });

            // 2. 更新當前項目的 timestamp 為目標 timestamp
            await PUBLIC_CONTENT_COLLECTION_REF.doc(docId).update({
                timestamp: targetTimestamp
            });

            saveMessage.textContent = '成員順序調整成功！';
            setTimeout(() => saveMessage.textContent = '', 3000);

        } catch (error) {
            console.error("移動操作錯誤: ", error);
            saveMessage.textContent = `移動失敗: ${error.message}.`;
        }
    }
</script>
...
```
<img width="1532" height="656" alt="image" src="https://github.com/user-attachments/assets/4e061767-db6f-45f1-b4fa-ac263042a571" />
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Firebase 圖文網格編輯範例 (可移動順序)</title>
    ...
    <style>
        ...
        .move-btn-container {
            display: flex;
            justify-content: space-between;
            width: 80px; /* 寬度與刪除按鈕對齊 */
            margin: 0 auto 5px auto; /* 底部間距 */
        }

        /* 🚨 移動按鈕 (細長設計) 🚨 */
        .action-btn.move-btn {
            background-color: #007bff; /* 藍色 */
            padding: 4px 6px; /* 縮小 Padding 實現更細長的按鈕 */
            width: 38px; /* 設定單個按鈕寬度 */
            margin: 0 !important; /* 移除原本的 margin-auto */
                    ...
                    <li>
                        ...
                        ${isLoggedIn ? 
                            `<div style="display: flex; justify-content: space-around; width: 100%;">
                                ${currentOrder > 1 ? 
                                    `<button class="action-btn move-btn" onclick="moveContent('${docId}', 'up', ${index})">
                                        <i class="fas fa-angle-left"></i> 
                                    </button>` : `<div style="width: 80px; margin: 0 5px 0 0;"></div>`}
                                    
                                ${currentOrder < totalItems ? 
                                    `<button class="action-btn move-btn" onclick="moveContent('${docId}', 'down', ${index})">
                                        <i class="fas fa-angle-right"></i>
                                    </button>` : `<div style="width: 80px; margin: 0 0 0 5px;"></div>`}
                            </div>` : ''}
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Firebase 圖文網格編輯範例 (完整版)</title>
    ...
    <style>
        ...
        .action-btn {
            ...
            width: 80px; /* 統一按鈕寬度 */
            padding: 5px 10px; 
            margin: 5px auto 0 auto !important; /* 確保垂直間距 */
        }
        ...
        /* 🚨 更新圖片按鈕 🚨 */
        .action-btn.update-image-btn {
            background-color: #ff9800; /* 橙色 */
            margin-top: 5px !important;
            margin-bottom: 5px !important;
        }
        .action-btn.update-image-btn:hover {
        ...
        .move-btn-container {
            display: flex;
            justify-content: space-between;
            width: 80px; /* 寬度與其他按鈕對齊 */
            margin: 5px auto 5px auto; /* 位於名稱和圖片更新按鈕之間 */
        }
...
<script>
                        ...
                        ${isLoggedIn ? 
                            ...
                            <div class="move-btn-container">
                                ${currentOrder > 1 ? 
                                    `<button class="action-btn move-btn" onclick="moveContent('${docId}', 'up', ${index})">
                                        <i class="fas fa-angle-left"></i> 
                                    </button>` : `<div style="width: 38px;"></div>`}
                            ...
                            <button class="action-btn update-image-btn" onclick="promptUpdateImage('${docId}', '${data.imageUrl || ''}')">
                                <i class="fas fa-image"></i> 更新圖片
                            </button>
    ...
    // ** 8. 觸發圖片更新的函數 (使用隱藏的 input) **
    window.promptUpdateImage = (docId, oldImageUrl) => {
        if (!auth.currentUser) {
            alert('您沒有權限執行此操作。');
            return;
        }
        ...
        fileInput.onchange = (e) => {
            ...
            if (newImageFile) {
                // 執行圖片更新流程
                updateImage(docId, newImageFile, oldImageUrl);
            }
        };
    ...
    // ** 9. 執行圖片上傳和資料庫更新 **
    async function updateImage(docId, newImageFile, oldImageUrl) {
        saveMessage.textContent = '正在上傳新圖片...';
        
        try {
            // 步驟 A: 上傳新圖片到 Cloud Storage
            const storageRef = storage.ref(`images/${Date.now()}_${newImageFile.name}`);
            const snapshot = await storageRef.put(newImageFile);
            const newImageUrl = await snapshot.ref.getDownloadURL(); 
            
            saveMessage.textContent = '新圖片上傳成功，正在更新資料庫...';

            // 步驟 B: 更新 Firestore 中的圖片 URL
            await PUBLIC_CONTENT_COLLECTION_REF.doc(docId).update({
            ...
            if (oldImageUrl) {
                try {
                    ...
                    // 如果舊圖片不存在或刪除失敗，只需記錄錯誤，不中斷主要流程
                    console.warn("無法刪除舊圖片 (可能不存在或已移動):", deleteError);
                }
            }
            
            saveMessage.textContent = '圖片更新成功！';
            setTimeout(() => saveMessage.textContent = '', 3000);

        } catch (error) {
            console.error("更新圖片錯誤: ", error);
            saveMessage.textContent = `圖片更新失敗: ${error.message}.`;
        }
    }
</script>
