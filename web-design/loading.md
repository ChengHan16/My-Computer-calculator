...
.loading-msg i {
    ...
    animation: fa-spin 2s infinite linear;
        ...
        /* --- 4. 關鍵優化：網頁版並排 3-4 個卡片 --- */
        .member-grid {
            display: grid !important; /* 強制啟動網格 */
            /* 電腦版預設：並排 4 個，每個佔 1 份等寬比例 */
        ...
        .member-image-wrapper {
            ...
            border-radius: 50%;
            overflow: hidden;
            border: 4px solid #020093; /* 黃色邊框 */
        }

        .member-image-wrapper02 {
            width: 100px;
            height: 140px;
            margin: 0 auto 15px;
            border-radius: 50%;
        ...
        .member-image-wrapper img {
            width: 100%;
            height: 100%;
            object-fit: cover; /* 關鍵：確保圖片填滿容器並保持比例 */
            display: block;
        }

        .member-image-wrapper02 img {
            width: 100%;
            height: 100%;
            object-fit: cover; /* 關鍵：確保圖片填滿容器並保持比例 */
            display: block;
        ...
        .member-name-placeholder {
            ...
            color: #004d99; /* 次深藍色 */
            margin-bottom: 10px;
            /* 修正：如果使用真實姓名，請將 class 名稱改成 .member-name */
        }
        ...
        @media (max-width: 768px) {
            ...
            .banner-info-wrap { /* 提示字*/
                margin-top: 10px;
                padding-bottom: 10px;
                font-size: 12px;
            }
            /* 手機版 標題列和黃字說明大小調整 End */
...
<body>
    ...
    <section class="event-banner">
        ...
        <div class="banner-info-wrap">
            <span><i class="fa-solid fa-calendar-check"></i> 團隊成員介紹</span>
            <span><i class="fa-solid fa-crown"></i> 人員編制說明</span>
        </div>
    </section>

    <main class="member-list" id="member-container">
...
<script>
    // 1. 您的 Firebase 配置 (請從 Firebase Console 複製)
    ...
    try {
        ...
        if (querySnapshot.empty) {
            console.warn("資料庫是空的！");
            container.innerHTML = "資料庫目前沒有成員。";
            return;
        }
        
        container.innerHTML = ""; // 清空
        querySnapshot.forEach(doc => {
            console.log("讀取到成員:", doc.data().name);
            // 渲染 HTML ...
        });
    } catch (error) {
...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // ** 2. 監聽 Firebase 資料 (即時更新) **
            db.collection("content").orderBy("timestamp", "asc").onSnapshot((snapshot) => {
                if (snapshot.empty) {
                    container.innerHTML = "<p style='text-align:center; width:100%; color:#999;'>資料庫目前無成員資料。</p>";
                    return;
                }
                ...
                snapshot.docs.forEach((doc, index) => {
                    ...
                    const imgSrc = data.imageUrl || 'https://github.com/ChengHan16/Legislature-Family-Website/blob/main/image/Mysterious01.png?raw=true';
                    const memberNumber = index + 1;

                    // 判斷是否為「守護神」(包含 Shiba 或 柴犬 或 吉祥物)
                    ...
                    // 生成與你原本 members.html 一模一樣的卡片格式
                    const cardHTML = `
                        <div class="member-item ${isGuardian ? 'guardian-member' : ''}">
                            <div class="member-name-placeholder">成員 ${memberNumber}</div>
                            <div class="member-image-wrapper" ${isGuardian ? 'style="border: 3px solid #ffcc00; box-shadow: 0 0 15px rgba(255, 204, 0, 0.5);"' : ''}>
                                <a href="info-${nickname}.html">
                                    <img src="${imgSrc}" alt="${nickname} 頭像">
                                </a>
                            </div>
                            <div class="member-title" ${isGuardian ? 'style="color: #bc9b00; font-weight: bold;"' : ''}>${nickname}</div>
                        </div>
                    `;
                ...
                container.innerHTML = `<p style='color:red;'>連線錯誤: ${error.message}</p>`;
            });
        });
    </script>
